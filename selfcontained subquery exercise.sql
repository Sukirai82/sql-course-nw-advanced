...
SELECT   pp.TransactionID,
         ...
         pp.PAON,
         pp.Street,
         (SELECT AVG(ranmere.Price)
          FROM   PricePaidSW12 AS ranmere
          WHERE  ranmere.Street = 'Ranmere Street') AS AveragePriceInRanmereStreet
FROM     PricePaidSW12 AS pp
WHERE    pp.Street = 'Ranmere Street'
         AND pp.Price > (SELECT AVG(ranmere.Price)
                         FROM   PricePaidSW12 AS ranmere
                         WHERE  ranmere.Street = 'Ranmere Street')
ORDER BY pp.Price;
