...
SELECT
    ...
    ELSE 0 END AS IsColourAndStyleMissing
FROM
    Message m;
...
SELECT
    m.Region
    ,COUNT(*) AS NumMessages
    ,SUM(CASE WHEN m.Colour IS not NULL and m.Style IS not NULL THEN 1 ELSE 0 END) AS NumMessagesWithColourAndStyleMissing
FROM
    Message m
GROUP BY
    m.Region;